...
public class PackagingCatalogService {
    ...
    public PagedResponse<PackagingCatalogDTO> findAll(int page, int size) {
        var query = catalogRepository.findAll();
        var list = query.page(Page.of(page, size)).list();
        return PagedResponse.of(list.stream().map(PackagingCatalogDTO::from).toList(), page, size, query.count());
    }
